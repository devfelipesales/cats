...
import { TFriends, fetchFriends } from "@/app/lib/fetchData";
...
export default function Friends({ loggedUser }: { loggedUser: string }) {
  const [data, setData] = useState<TFriends[] | null>();
  const isComponentMounted = useRef(false);

  useEffect(() => {
    isComponentMounted.current = true;
    async function getData() {
      const data = await fetchFriends(loggedUser);
      if (data) {
        setData(data);
      }
    }
  ...
  async function handleSubmit(event: React.FormEvent<CustomForm>) {
    ...
    if (id) {
      ...
      const data = await removeFriend(id.value);
    }
  }
      ...
      <div className="grid grid-cols-2 place-items-center gap-x-8 gap-y-10 sm:grid-cols-3">
        {data &&
          data.map((item) => {
            ...
            profile = item.Added.profile;
            if (item.Added.profile === loggedUser) {
              name = item.AddedBy.name;
              profile = item.AddedBy.profile;
              image = item.AddedBy.image;
            } else {
              name = item.Added.name;
              profile = item.Added.profile;
              image = item.Added.image;
            }
            return (
              <form
                key={item.id}
